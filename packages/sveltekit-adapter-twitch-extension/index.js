...
import { postProcessTwitchExtensionBuild } from './twitch-postprocess.js';

/** @type {import('./index.js').default} */
export default function (options) {
	return {
		name: 'sveltekit-adapter-twitch-extension',
		...
		async adapt(builder) {
			if (!options?.fallback && builder.config.kit.router?.type !== 'hash') {
				...
				if (dynamic_routes.length > 0 && options?.strict !== false) {
					...
					builder.log.error(
						`sveltekit-adapter-twitch-extension: all routes must be fully prerenderable, but found the following routes that are dynamic:
...
If this doesn't help, you may need a different adapter. This adapter extends adapter-static and can only be used for sites that don't need a server for dynamic rendering, and can run on just a static file server.
See https://svelte.dev/docs/kit/page-options#prerender for more details`
			...
			postProcessTwitchExtensionBuild({
				pages,
				log: builder.log,
				createZip: createZip === true,
				removeStatic: removeStatic !== false
			});
